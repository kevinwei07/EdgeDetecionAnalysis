...
checkpoint_dir = 'RCF/pretrained/RCFcheckpoint_epoch12.pth' # original
#checkpoint_dir = 'RCF/pretrained/epoch-9-checkpoint.pth' # try canny
#checkpoint_dir = 'RCF/pretrained/epoch-1-checkpoint.pth' # try clean gt 
...
def make_single_rcf(image, edge_img_path, enlarge, padding_size):
    ...
    while True:
        try:
            resi_image = cv2.resize(image, (0, 0), fx=resize_factor, fy=resize_factor)
            # TODO add guassian / bilateral filter
            resi_image = np.transpose(resi_image, (2, 0, 1))                                  # (3, 736, 550)
            resi_image = torch.FloatTensor(resi_image).unsqueeze(0)
            resi_image = resi_image.cuda()
            results = model(resi_image)
