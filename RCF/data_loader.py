...
class BSDS_RCFLoader(data.Dataset):
    ...
    def __init__(self, root='data/HED-BSDS_PASCAL', split='train', transform=False):
        ...
        # elif self.split == 'tune':
            self.filelist = join(self.root, 'tune.lst')
        elif self.split == 'test':
            self.filelist = join(self.root, 'test.lst') # (321,481) / (481,321)
        else:
    ...
    def __getitem__(self, index):
        if self.split == "train":
            img_file, lb_file = self.filelist[index].split() # lb refers to label
            #print(img_file)
            #print(lb_file)
            #lb = np.array(Image.open('data/HED-BSDS_PASCAL/tune/data_gt/pipe (14)_1_Canny-1-1_Threshold.jpg'), dtype=np.float32)
            #lb = np.array(Image.open(join(self.root, lb_file.rstrip('\n'))).resize((1600,1200),Image.BILINEAR), dtype=np.float32)
